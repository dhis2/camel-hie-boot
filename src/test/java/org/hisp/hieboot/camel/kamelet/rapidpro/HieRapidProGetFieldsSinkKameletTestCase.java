...
public class HieRapidProGetFieldsSinkKameletTestCase extends AbstractHieRapidProKameletTestCase {
    ...
    @Test
    public void testHieGetFieldsRapidProSinkGivenNextPageUrl() throws Exception {
        ...
        MockEndpoint rapidProEndpoint = camelContext.getEndpoint("mock:rapidpro/fields.json?httpMethod=GET&okStatusCodeRange=200-299", MockEndpoint.class);
        rapidProEndpoint.setExchangePattern(ExchangePattern.InOut);
        rapidProEndpoint.whenExchangeReceived(1, exchange -> {
            Map<String, Object> results = Map.of("next", "mock:rapidpro/fields.json", "results", List.of(Map.of("key", "foo", "name", "foo", "type", "text")));
        ...
        rapidProEndpoint.whenExchangeReceived(2, exchange -> {
            Map<String, Object> results = Map.of("results", List.of(Map.of("key", "bar", "name", "bar", "type", "text")));
            exchange.getMessage().setBody(new ObjectMapper().writeValueAsString(results));
            exchange.getMessage().setHeader("CamelHttpResponseCode", 200);
        });
        ...
        assertEquals("foo", body.get("key"));
        assertEquals("foo", body.get("name"));

        body = verifyEndpoint.getReceivedExchanges().get(1).getMessage().getBody(Map.class);
        assertEquals("text", body.get("type"));
        assertEquals("bar", body.get("key"));
        assertEquals("bar", body.get("name"));
    }

    @Test
    public void testHieGetFieldsRapidProSink() throws Exception {
        camelContext.addRoutes(new RouteBuilder() {
            @Override
            public void configure() {
                from("direct:routeUnderTest")
    ...
    @Test
    public void testHieGetFieldsRapidProSinkByKey() throws Exception {
        camelContext.addRoutes(new RouteBuilder() {
            @Override
            public void configure() {
                from("direct:routeUnderTest")
        ...
        assertEquals("foo", body.get("label"));
    }
